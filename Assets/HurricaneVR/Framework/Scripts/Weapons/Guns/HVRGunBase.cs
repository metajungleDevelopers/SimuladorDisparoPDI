...
namespace HurricaneVR.Framework.Weapons.Guns
{
    public class HVRGunBase : HVRDamageProvider
    {
        ...
        [Header("Impact")]
        public GameObject impactSpritePrefab; // Prefab con un SpriteRenderer del impacto
        public GameObject impactRagdollPrefab;
        ...
        protected virtual void OnHit(RaycastHit hit, Vector3 direction)
        {
            ...
            {
                // Verifica la layer para saber qué impacto crear
                if (hit.collider.gameObject.layer == LayerMask.NameToLayer("RagdollHit"))
                {
                    // Instancia impacto de sangre (o lo que quieras)
                    if (impactRagdollPrefab != null)
                    {
                        GameObject bloodImpact = Instantiate(impactRagdollPrefab, hit.point + hit.normal * 0.01f, Quaternion.LookRotation(-hit.normal));
                        bloodImpact.transform.SetParent(hit.collider.transform, true);
                    }
                }
                else
                {
                    // Instancia impacto normal
                    if (impactSpritePrefab != null)
                    {
                        GameObject impact = Instantiate(impactSpritePrefab, hit.point + hit.normal * 0.01f, Quaternion.LookRotation(-hit.normal));
